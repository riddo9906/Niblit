...
_EVOLVE_EVERY: int = 3
# Default roles spawned at civilization start.
...
def _make_typed_agent(agent_id: str, role: str) -> Any:
    ...
    try:
        ...
        log.debug("CivilizationController: typed agent creation failed (%s), using BaseAgent fallback: %s", role, exc)
        try:
            from civilization.agent_population.base_agent import BaseAgent
...
class CivilizationController:
    ...
    def __init__(self, initial_roles: Optional[List[str]] = None) -> None:
    ...
    def run_cycle(self) -> Dict[str, Any]:
        ...
        for agent_meta in agents:
            ...
            # 5. Publish result on MessageBus; harvest research insights ───────
                    ...
                    if self._knowledge_api:
                        try:
                            self._knowledge_api.store_knowledge(
                                text, tags=["civilization", "research", f"cycle_{self._cycle_count}"]
                            )
                        except Exception:
                            pass
    ...
    def to_findings_dict(self) -> Dict[str, Any]:
        ...
        # Clear consumed insights so next call doesn't double-ingest
