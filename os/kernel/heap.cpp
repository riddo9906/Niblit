...
static constexpr size_t SLAB_SIZES[]  = { 8, 16, 32, 64, 128, 256, 512 };
static constexpr size_t NUM_SLABS     = sizeof(SLAB_SIZES) / sizeof(SLAB_SIZES[0]);
static constexpr size_t OBJECTS_PER_PAGE = Memory::PAGE_SIZE / 512; // conservative
...
// ── Large allocation header (stored just before the user data) ────────────────
// For large (page-sized) allocs we prepend a header in the same page(s).
struct LargeHeader {
...
// ── Slab helper ───────────────────────────────────────────────────────────────
static void replenish(SlabCache* sc) {
    // Allocate one page and carve it into fixed-size objects.
    uint32_t frame = Memory::alloc_frame();
    if (!frame) return;

    uint8_t* p   = reinterpret_cast<uint8_t*>(frame);
    size_t   cnt = Memory::PAGE_SIZE / sc->obj_size;

    for (size_t i = 0; i < cnt; ++i) {
        auto* node   = reinterpret_cast<FreeNode*>(p + i * sc->obj_size);
        node->next   = sc->free_list;
        sc->free_list = node;
    }
}

static SlabCache* find_slab(size_t size) {
    for (size_t i = 0; i < NUM_SLABS; ++i) {
        if (s_slabs[i].obj_size >= size) return &s_slabs[i];
    }
    return nullptr;
...
void* kmalloc(size_t size) {
    ...
    SlabCache* sc = find_slab(size);
    if (sc) {
        if (!sc->free_list) replenish(sc);
        if (!sc->free_list) return nullptr; // OOM
    ...
    // Large allocation: prepend header in first page, rest for data.
    size_t total     = sizeof(LargeHeader) + size;
    size_t num_pages = (total + Memory::PAGE_SIZE - 1) / Memory::PAGE_SIZE;

    // We only support contiguous physical allocation via repeated alloc_frame
    // for simplicity (no buddy system yet).
    void* first = nullptr;
    uint8_t* ptr = nullptr;
    for (size_t i = 0; i < num_pages; ++i) {
        uint32_t frame = Memory::alloc_frame();
        if (!frame) return nullptr;
        if (i == 0) {
            first = reinterpret_cast<void*>(frame);
            ptr   = reinterpret_cast<uint8_t*>(frame);
        }
    }
    ...
    s_used += num_pages * Memory::PAGE_SIZE;
    return ptr + sizeof(LargeHeader);
}

void kfree(void* ptr) {
    if (!ptr) return;

    // Check if it's a large allocation (header immediately before ptr).
    auto* hdr = reinterpret_cast<LargeHeader*>(
        reinterpret_cast<uint8_t*>(ptr) - sizeof(LargeHeader));
    if (hdr->magic == HEAP_MAGIC) {
        // Free all pages.
    ...
    // Must be a slab object — find the matching slab by pointer alignment.
    for (size_t i = 0; i < NUM_SLABS; ++i) {
        // Return to slab free list.
        auto* node    = reinterpret_cast<FreeNode*>(ptr);
        node->next    = s_slabs[i].free_list;
        s_slabs[i].free_list = node;
        --s_slabs[i].allocated;
        s_used -= s_slabs[i].obj_size;
        return;
    }
}
