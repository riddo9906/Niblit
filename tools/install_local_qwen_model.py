...
def _show_setup_instructions(gguf_path: str) -> None:
    ...
    termux_dest = dest.replace("~", "/data/data/com.termux/files/home", 1)
    print()
    print("=" * 60)
    print("  One-time setup: GGUF model + llama.cpp binary")
    print("=" * 60)
    print()
    print("── Step 1: Download the model (~470 MB) ────────────────────")
    print()
    ...
    print("  cd ~/llama.cpp && mkdir -p build && cd build")
    print("  cmake .. -DLLAMA_NATIVE=OFF -DLLAMA_BUILD_TESTS=OFF")
    print("  cmake --build . -j1")
    print()
    print("── Step 3: Choose your session layout ───────────────────────")
    print()
    print("  Option A — Two Termux sessions (recommended for proot):")
    print("    Session 1 (native Termux): run llama-server")
    print("      ~/llama.cpp/build/bin/llama-server \\")
    print(f"          -m {dest} \\")
    print("          --port 8080 --host 127.0.0.1 -c 2048 -t 4")
    print()
    print("    Session 2 (Termux proot): run Niblit")
    print(f"      export NIBLIT_GGUF_MODEL_PATH={dest}")
    print("      export NIBLIT_GGUF_BACKEND=http")
    print("      export NIBLIT_LLAMA_SERVER_URL=http://127.0.0.1:8080")
    print("      python main.py")
    print()
    print("  Option B — Single Termux session (same session, no proot):")
    print(f"      export NIBLIT_GGUF_MODEL_PATH={dest}")
    print("      export NIBLIT_GGUF_BACKEND=subprocess")
    print("      # Use absolute path when calling from inside proot:")
    print(f"      export NIBLIT_LLAMA_BINARY={termux_dest.replace('/models/qwen2.5-0.5b-instruct-q4_k_m.gguf', '/llama.cpp/build/bin/llama-cli')}")
    print("      python main.py")
    print()
    print("── Step 4: Verify ───────────────────────────────────────────")
    print()
