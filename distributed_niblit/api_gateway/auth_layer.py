...
class AuthLayer:
    ...
    def __init__(self) -> None:
        # HMAC secret used to derive a keyed digest of each API key before
        # storage.  A per-deployment secret ensures stored digests cannot be
        # reversed if the key table is leaked.  Supply AUTH_KEY_HASH_SECRET via
        # the environment in production; a random per-process secret is used
        # when the variable is absent (keys will not survive restarts).
        env_secret = os.getenv("AUTH_KEY_HASH_SECRET")
        if env_secret:
            self._hash_secret: bytes = env_secret.encode()
        else:
            self._hash_secret = secrets.token_bytes(32)
            log.warning(
    ...
    def _hash_api_key(self, raw_data: str) -> str:
        """Return a keyed HMAC digest of the supplied material for safe storage."""
        return hmac.new(self._hash_secret, raw_data.encode(), "sha256").hexdigest()
