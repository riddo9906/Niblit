...
Uses GGUF quantized models via ``llama-cpp-python`` — the only supported
backend.  Quantized GGUF models (e.g. q4_K_M) use ~300–600 MB RAM with no
PyTorch dependency, making them stable on Android/Termux and lightweight on
any CPU-only device.
...
The model is cached for the process lifetime.  The ``llama_cpp`` import is
guarded so the module loads cleanly even without ``llama-cpp-python``.
...
class QwenLocalBrain:
    """CPU-friendly local LLM brain using GGUF quantized models via llama-cpp-python.

    Requires ``llama-cpp-python`` (``pip install llama-cpp-python``).
    Set ``NIBLIT_GGUF_MODEL_PATH=/path/to/model.gguf`` or point
    ``NIBLIT_LOCAL_MODEL`` at a ``.gguf`` file path.
    ...
    def _ensure_loaded(self) -> bool:
        """Load the GGUF model if not yet done.  Returns True on success."""
