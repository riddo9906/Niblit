...
class CodeGenerator:
    ...
    def generate_from_research(
        ...
        if not name:
            safe = re.sub(r"[^a-z0-9]+", "_", topic.lower())[:_MAX_MODULE_NAME_LEN].strip("_")
            name = f"ale_{lang}_{safe}" if safe else f"ale_{lang}_module"
    ...
    def _generate_python_from_research(
        ...
        summary = research_text[:300].replace('"""', "'''").strip()
    ...
    def _generate_rust_from_research(
        ...
        summary = textwrap.shorten(
            research_text.replace('"', "'"), width=120, placeholder="..."
        )
    ...
    def _generate_go_from_research(
        ...
        pkg = re.sub(r"[^a-z0-9]+", "", name.lower())[:20] or "alemodule"
        struct_name = classname if classname else "AleModule"
