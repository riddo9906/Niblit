...
class AutonomousLearningEngine:
    ...
    def _autonomous_code_generation(self) -> str:
        ...
        The resulting code is queued for compilation in step 10 and saved to
        both the deploy path (hot-reload) and the builds/ directory.
        """
        code_gen = self._get_code_generator()
        llm = self._get_llm()
        ...
        # ── Collect all recent research context from KB ─────────────────────
        # Gather facts from step 8 (ale_code_research:, ale_internet_code:)
        # and prior reflections (ale_code_learning:) to build a rich prompt.
        research_snippets: List[str] = []
        docstring = f"Auto-generated by Niblit ALE — {lang} module for {topic}"
        if self.knowledge_db:
            try:
                facts = (
                    self.knowledge_db.list_facts(30)
                ...
                for f in reversed(facts):
                    ...
                    if any(key.startswith(p) for p in (
                        f"ale_code_research:{lang}:",
                        f"ale_internet_code:{lang}:",
                        f"ale_code_learning:{lang}:",
                    )):
                        snippet = val[:_MAX_RESEARCH_SNIPPET_LENGTH].strip()
                        if snippet and snippet not in research_snippets:
                            research_snippets.append(snippet)
                    if len(research_snippets) >= 5:
                        break
            except Exception:
                pass
        ...
        if research_snippets:
            ...
            docstring = f"Based on research: {research_snippets[0][:100]}"
        else:
            research_context = ""

        # ── Code generation: LLM-powered or template fallback ───────────────
        module_name = f"ale_{lang}_{topic}"
        classname = "".join(w.capitalize() for w in module_name.split("_"))
        purpose = (
            f"implement a {lang} module named '{module_name}' that applies these "
            f"best practices: {research_context[:200] or topic}"
        )

        try:
            if hasattr(code_gen, "generate_with_llm"):
                result = code_gen.generate_with_llm(
                    lang,
                    purpose,
            ...
            log.info("✅ [CODE GEN] Generated %s code via %s (%d chars)", lang, source, len(code))

            # ── Idea-driven enrichment ─────────────────────────────────────
            # When LLM was used we already have real logic; skip text-plan
            # appending which previously produced non-executable comment blocks.
            if source == "template" and self.idea_implementation and hasattr(
                self.idea_implementation, "implement_idea"
            ):
                try:
                    idea_prompt = f"Generate a {lang} utility for: {topic}"
                    impl_result = self.idea_implementation.implement_idea(idea_prompt)
                    if impl_result:
                        # Only append if the result looks like actual code (not a
                        # plain text plan).  Heuristic: real code contains at least
                        # one line that is NOT a comment.
                        lines = [
                            ln for ln in str(impl_result).splitlines()
                            if ln.strip() and not ln.strip().startswith("#")
                        ]
                        if lines:
                            code += f"\n{impl_result}"
                        else:
                            # Log the plan but do not pollute generated source
                            log.debug("[CODE GEN] Idea plan (template-only): %s", str(impl_result)[:120])
                except Exception as exc:
                    log.debug(f"Idea-driven generation failed: {exc}")

            # ── Structural correction on final code ────────────────────────
            ...
            # ── Persist to deploy path (Python hot-reload) ─────────────────
            deploy_note = ""
            ...
            return (
                f"Code generated: {lang} module via {source} ({len(code)} chars)"
                f" — queued for compilation{deploy_note}{build_note}"
            )
