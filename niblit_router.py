            ...
            try:
                ...
                if self.core is not None:
                    ...
                    try:
                        ...
                        log.exception("[LocalModel] Unexpected local_brain wire error: %s", exc)
                st = new_lb.status()
                return (
    ...
    def help_text(self):
        ...
        if registry is not None:
            ...
            try:
                ...
                log.debug("Command registry help unavailable, falling back to static help: %s", exc)
        commands = [
            "╔══════════════════════════════════════════════════════════════════════╗",
            ...
            "sa-scripts                    — Runtime scripts and automation status",
            "sa-awareness                  — All structural awareness in one view",
            "",
            "=== DEV AGENT ===",
            "dev-agent status              — Dev-agent status snapshot",
            "dev-agent runtime             — Dev-agent runtime status",
            "dev-agent providers           — Dev-agent providers/tooling overview",
            "dev-agent architecture        — Dev-agent architecture report",
            "dev-agent analyze <task>      — Analyze task and produce plan",
            "dev-agent approve <id>        — Approve staged mutation",
            "dev-agent execute <id>        — Execute approved mutation",
            "dev-agent rollback [id]       — Roll back latest/specified mutation",
            "",
            "=== CODE GENERATION ===",
            ...
            "read build file <name>       — Read a specific build file (explicit form)",
            ...
            "deploy-bridge [status|save|load] — Deployment bridge state control",
            "autonomous-network [status|start|stop|reflect] — Network autonomy control",
            "module-autonomy [status|start|stop|module <name>] — Module autonomy control",
            "coherence                    — Temporal coherence status",
            "",
