...
class HFBrain:
    ...
    def __init__(self, db):
        ...
        # Lightweight cache for the KB snapshot so repeated prompts do not
        # re-read the same facts from the database every turn.
        self._kb_snapshot_cache: str | None = None
        self._kb_snapshot_cache_expires_at = 0.0
        self._kb_snapshot_cache_ttl_seconds = 30.0
    ...
    def _build_kb_snapshot(self) -> str:
        ...
        """
        now = time.monotonic()
        if self._kb_snapshot_cache is not None and now < self._kb_snapshot_cache_expires_at:
            return self._kb_snapshot_cache
        ...
        try:
            ...
            try:
                facts = self.db.list_facts(200) if hasattr(self.db, "list_facts") else []
            except Exception:
                return ""
        except Exception:
            return ""
        ...
        self._kb_snapshot_cache = snapshot
        self._kb_snapshot_cache_expires_at = now + self._kb_snapshot_cache_ttl_seconds
        return snapshot
